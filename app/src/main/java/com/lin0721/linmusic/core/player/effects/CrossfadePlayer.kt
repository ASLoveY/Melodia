...
class CrossfadePlayer(
    ...
    private val decks = Array(2) { index ->
        val renderers = object : DefaultRenderersFactory(context) {
            override fun buildAudioSink(context: Context, enableFloatOutput: Boolean, enableAudioTrackPlaybackParams: Boolean): AudioSink =
                DefaultAudioSink.Builder(context).setEnableFloatOutput(false).setAudioProcessors(arrayOf(processors[index])).build()
        }
        ExoPlayer.Builder(context, renderers).setMediaSourceFactory(DefaultMediaSourceFactory(dataSource)).build().apply {
    ...
    val activeDeck: ExoPlayer get() = decks[activeIndex]
    ...
    init {
        decks.forEach { deck -> deck.addListener(object : Player.Listener {
            override fun onMediaItemTransition(mediaItem: MediaItem?, reason: Int) {
                if (deck === activeDeck && reason == Player.MEDIA_ITEM_TRANSITION_REASON_REPEAT) processors[activeIndex].complete(deck.duration)
            }
            override fun onEvents(player: Player, events: Player.Events) {
                if (disposed || suppressEvents) return
                if (player === activeDeck) {
                    if (player.playbackState == Player.STATE_ENDED) {
                        processors[activeIndex].complete(player.duration)
    ...
    fun prepareNext(value: PreparedTransition) {
        if (!effects.crossfadeEnabled || !isTransitionCurrent(value) || value.currentKey != activeDeck.currentMediaItem?.mediaId || activeDeck.repeatMode == Player.REPEAT_MODE_ONE) return
        cancelPreparation()
        prepared = value
    ...
    fun setPreferredAudioDevice(device: AudioDeviceInfo?) { decks.forEach { it.setPreferredAudioDevice(device) } }
