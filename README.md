...
预发布版 **v1.7.0-beta.2** 修复开启背景图片后的[搜索联想文字叠加](docs/SEARCH_OVERLAP_FIX.md)，并包含 beta.1 的 35% 衬底、蓝牙 LDAC 高精度实验模式及每日推荐接口回退与空状态修复。已用真实账号验证空日推、歌单和在线播放；实体 LDAC 链路待验证，实验模式不能强制手机切换 LDAC。[使用与验收说明](docs/LDAC_BETA_TESTING.md)。
