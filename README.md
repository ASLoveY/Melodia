...
预发布版 **v1.7.0-beta.3** 将 LDAC 实验模式接入系统原生引擎，已在 OnePlus 13s / Android 16 + WF-1000XM5 上验证从 48 kHz 混音切换至 DIRECT / 96 kHz 输出；[实现、验收与适用范围](docs/NATIVE_DIRECT_OUTPUT.md)。包含此前的 35% 衬底、[搜索联想叠字修复](docs/SEARCH_OVERLAP_FIX.md)和每日推荐空状态修复。实验模式不能强制 LDAC 或保证所有设备直出；非空日推仍待真实账号验证。

- **平滑播放**：自动切歌默认使用 3 秒交叉淡化，可在“设置 → 播放与缓存”关闭或调整为 1–12 秒；手动切歌立即响应。响度均衡默认开启，首次播放逐步估算，完整播放后复用分析结果，尽量保留音乐强弱变化。
- **自定义背景**：在设置中选择本地图片，支持更换、恢复默认与 0–100% 透明度调节。图片保存为应用内部副本，适用于常规页面；完整播放器保留专辑背景，页面保留文字可读性衬底。
- **背景显示修复（v1.6.1）**：首页顶部与信息流共用完整背景；减弱衬底并增强文字对比度。导入图片先完整读取，拒绝不完整解码，避免保存成残缺背景。

- **搜索歌曲操作**：长按单曲或点击右侧更多按钮，选择下一首播放、加入队列末尾、添加到歌单、下载到本地、查看歌手/专辑或分享。多歌手可分别选择；添加歌单和下载需要登录，下载权限以服务端结果为准。下载完成后自动进入本地音乐，支持离线播放。
- **外观与主题**：首页右上角设置入口无需登录即可使用；支持浅色、深色及跟随系统，选择会自动保存。正文、次级文字、筛选和弹窗使用对应主题的高对比配色。
- **在线音乐库**：仅展示用户创建、订阅或收藏的歌单、关注歌手和收藏专辑。在歌单更多操作中可删除自建歌单或取消收藏；“我喜欢的音乐”受到保护。
- **个人资料**：从侧栏点击头像进入资料页，可查看账号资料并编辑昵称和签名。
- **本地音乐**：支持文件及目录导入，递归扫描并忽略不足 30 秒的音频。长按歌曲后可拖选、全选当前筛选结果并批量移出；在“已导入目录”中可移除目录及关联歌曲。支持搜索、排序、离线播放，所有移除操作均保留设备上的原文件。

Hi-Res 音频输出已列为后续方向，当前尚未实现高精度输出模式；参见 [Android 14–17 调研与路线](docs/HI_RES_AUDIO_RESEARCH.md)。
