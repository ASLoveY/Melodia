...
- **音乐库歌单管理**：在歌单的更多操作中，创建的歌单可删除，收藏的他人歌单可取消收藏；“我喜欢的音乐”受到保护，不提供删除操作。
- **个人资料**：从侧栏点击头像进入资料页，可查看账号资料并编辑昵称和签名。
- **本地音乐**：支持选择音频文件或整个目录；目录导入会递归扫描子目录，忽略不足 30 秒的音频，并显示进度、重复和跳过数量，扫描期间可取消。支持搜索、排序和离线播放；移除只删除库记录，保留原文件。
